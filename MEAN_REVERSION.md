...
当前 `conf_okx_multi.yml` 启用 BTC、ETH、SOL、XRP、DOGE、SUI、SNDK、ZEC、OKB、ADA 十个 USDT 永续品种，使用项目已有的加密货币配置和当前均值回归实现，杠杆均为 3 倍。原科技股配置保留，但不在当前启动列表中。
...
| **金额止盈** | **每笔仓位价格浮盈 ≥ 0.75 USDT，立即发起市价平仓** |
| 浮盈口径 | 触发用 `trade_pnl_quote`：按可平仓方向的盘口价格计算，不扣手续费，也不把多个仓位相加。成交后的真实盈亏才用 `net_pnl_quote = trade_pnl_quote - 已计手续费` |
...
金额止盈、均值止盈、止损和超时退出以先触发者为准，因此也可能在价格浮盈不足 0.75 USDT 时因回归均值而退出。金额止盈触发后不会等待进一步回归。0.75 USDT 只比较价格浮盈；市价成交后记入的真实盈亏是 `net_pnl_quote`，还会受手续费、滑点和网络延迟影响。
...
实际使用的是 `conf/controllers/conf_okx_pmm_*.yml`。参数模板另存于 `strategy_configs/okx_mean_reversion/`，纳入版本管理。
