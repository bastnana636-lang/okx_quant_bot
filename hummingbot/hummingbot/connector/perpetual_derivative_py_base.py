...
class PerpetualDerivativePyBase(ExchangePyBase, ABC):
    ...
    async def _execute_set_leverage(self, trading_pair: str, leverage: int):
        # Setting an absolute leverage is idempotent; retry transient transport failures only.
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    self.logger().network(
                        f"Error setting leverage {leverage} for {trading_pair} after 3 attempts.",
                        exc_info=True,
                    )
                    return
                self.logger().warning(
                    f"Connection error setting leverage {leverage} for {trading_pair}. Retrying in 5 seconds...",
                    exc_info=True,
                )
                await self._sleep(5)
        if success:
        ...
        for trading_pair in self.trading_pairs:
            while True:
                try:
                    ...
                    self.logger().warning(
                        f"Error initializing funding info for {trading_pair}. Retrying in 5 seconds...",
                        exc_info=True,
                    )
                    await self._sleep(5)
