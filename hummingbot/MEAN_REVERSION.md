...
当前 `conf_okx_multi.yml` 启用 BTC、ETH、SOL、XRP、DOGE、SUI、ONE、ZEC、UNI、ADA 十个 USDT 永续品种，使用项目已有的加密货币配置和当前均值回归实现，杠杆均为 3 倍。原科技股配置保留，但不在当前启动列表中。
...
`total_amount_quote` 是名义仓位上限，不会再乘以杠杆。当前十个品种的配置合计 615 USDT；0.5 USDT 现金止盈、最低 0.6% 止损、0.12% 成本和最低收益风险比 1 会把单笔预算进一步限制到不超过 `0.5 / (0.006 + 0.0012) ≈ 69.44 USDT`，因此十个品种按最低止损计算的名义仓位合计上限约为 558.89 USDT，3 倍杠杆对应初始保证金约 186.30 USDT（不含额外费用和交易所保证金调整）。ATR 止损更宽时，预算会继续缩小。
...
| **金额止盈** | **每笔仓位净浮盈 ≥ 0.5 USDT，立即发起市价平仓** |
...
| 现金止盈约束 | 必要时缩小仓位，使预估止损含成本不超过 `0.5 / min_reward_risk` USDT |
...
金额止盈、均值止盈、止损和超时退出以先触发者为准，因此也可能在浮盈不足 0.5 USDT 时因回归均值而退出。金额止盈触发后不会等待进一步回归。0.5 USDT 是触发条件，市价成交后的净利润还会受剩余手续费、滑点和网络延迟影响。
