...
class MeanReversionSettings(BaseModel):
    ...
    require_closed_candle_confirmation: bool = Field(default=False)
...
class ReversionSnapshot:
    ...
    @property
    def closed_z(self) -> Decimal:
        return self.closed_z_score

    @property
    def live_z(self) -> Decimal:
        return self.z_score
...
def calculate_snapshot(candles: pd.DataFrame, now: float, mid: Decimal,
    ...
    live_z = (mid - mean) / std
    if settings.require_closed_candle_confirmation:
        signal = reversion_side(closed_z, settings)
        if reversion_side(live_z, settings) != signal:
            signal = 0
    else:
        signal = reversion_side(live_z, settings)
