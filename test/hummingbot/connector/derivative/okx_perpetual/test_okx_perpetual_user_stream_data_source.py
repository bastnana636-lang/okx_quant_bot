...
class OkxPerpetualUserStreamDataSourceTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def test_failed_reconnect_does_not_remove_previous_websocket_twice(self):
        first_ws, recovered_ws = AsyncMock(), AsyncMock()
        connect = AsyncMock(side_effect=[first_ws, ConnectionError("reconnect failed"), recovered_ws])
        process = AsyncMock(side_effect=[ConnectionError("connection lost"), asyncio.CancelledError()])
        with (
            patch.object(self.data_source, "_get_connected_websocket_assistant", connect),
            patch.object(self.data_source, "_subscribe_to_channels", AsyncMock()),
            patch.object(self.data_source, "_process_websocket_messages", process),
            patch.object(self.data_source, "_sleep", AsyncMock()),
        ):
            with self.assertRaises(asyncio.CancelledError):
                await self.data_source._listen_for_user_stream_on_url("test_url", asyncio.Queue())

        self.assertEqual(3, connect.await_count)
        self.assertEqual(2, process.await_count)
        first_ws.disconnect.assert_awaited_once()
        recovered_ws.disconnect.assert_awaited_once()
        self.assertEqual([], self.data_source._ws_assistants)
