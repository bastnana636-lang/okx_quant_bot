...
class TestMarketMakingControllerBase(IsolatedAsyncioWrapperTestCase):
    ...
    def test_get_not_active_levels_ids_follows_trend(self):
        self.controller.processed_data = {"trend": "UP"}
        self.assertEqual(
            self.controller.get_not_active_levels_ids([]),
            ["buy_0", "buy_1"],
        )
        self.controller.processed_data = {"trend": "DOWN"}
        self.assertEqual(
            self.controller.get_not_active_levels_ids([]),
            ["sell_0", "sell_1"],
        )
        self.controller.processed_data = {"trend": "SIDEWAYS"}
        self.assertEqual(
            self.controller.get_not_active_levels_ids([]),
            ["buy_0", "buy_1", "sell_0", "sell_1"],
        )

    def test_level_amount_uses_active_side_weights(self):
        self.mock_controller_config.buy_spreads = [0.002, 0.004, 0.006]
        self.mock_controller_config.sell_spreads = [0.002, 0.004, 0.006]
        self.mock_controller_config.buy_amounts_pct = [Decimal("40"), Decimal("35"), Decimal("25")]
        self.mock_controller_config.sell_amounts_pct = [Decimal("40"), Decimal("35"), Decimal("25")]
        self.mock_controller_config.total_amount_quote = Decimal("100")

        self.controller.processed_data = {"trend": "UP"}
        self.assertEqual(self.controller._get_level_amount_quote("buy_0"), Decimal("40"))
        self.assertEqual(self.controller._get_level_amount_quote("buy_1"), Decimal("35"))
        self.assertEqual(self.controller._get_level_amount_quote("buy_2"), Decimal("25"))

        self.controller.processed_data = {"trend": "SIDEWAYS"}
        self.assertEqual(self.controller._get_level_amount_quote("buy_0"), Decimal("20"))

    def test_get_price_and_amount_uses_spread_multiplier(self):
        self.controller.processed_data = {
            "reference_price": Decimal("100"),
            "spread_multiplier": Decimal("1.5"),
            "trend": "SIDEWAYS",
        }
        price, amount = self.controller.get_price_and_amount("buy_0")
        self.assertEqual(price, Decimal("100") * (Decimal("1") - Decimal("0.01") * Decimal("1.5")))
        self.assertGreater(amount, Decimal("0"))

    def test_get_triple_barrier_for_level(self):
        self.mock_controller_config.level_take_profits = [0.006, 0.008, 0.010]
        self.mock_controller_config.level_stop_losses = [0.012, 0.015, 0.020]
        self.mock_controller_config.trailing_stop = TrailingStop(
            activation_price=Decimal("0.006"), trailing_delta=Decimal("0.003")
        )
        barrier = self.controller.get_triple_barrier_for_level("buy_0")
        self.assertEqual(barrier.take_profit, Decimal("0.006"))
        self.assertEqual(barrier.stop_loss, Decimal("0.012"))
        self.assertEqual(barrier.trailing_stop.activation_price, Decimal("0.006"))

        barrier = self.controller.get_triple_barrier_for_level("sell_2")
        self.assertEqual(barrier.take_profit, Decimal("0.010"))
        self.assertEqual(barrier.stop_loss, Decimal("0.020"))

    def test_executors_to_early_stop_cancels_opposite_unfilled(self):
        buy_executor = MagicMock(spec=ExecutorInfo)
        buy_executor.is_active = True
        buy_executor.is_trading = False
        buy_executor.id = "buy-exec"
        buy_executor.custom_info = {"level_id": "buy_0"}

        sell_executor = MagicMock(spec=ExecutorInfo)
        sell_executor.is_active = True
        sell_executor.is_trading = False
        sell_executor.id = "sell-exec"
        sell_executor.custom_info = {"level_id": "sell_0"}

        filled_sell = MagicMock(spec=ExecutorInfo)
        filled_sell.is_active = True
        filled_sell.is_trading = True
        filled_sell.id = "filled-sell"
        filled_sell.custom_info = {"level_id": "sell_1"}

        self.controller.executors_info = [buy_executor, sell_executor, filled_sell]
        self.controller.processed_data = {"trend": "UP"}
        actions = self.controller.executors_to_early_stop()
        self.assertEqual(len(actions), 1)
        self.assertEqual(actions[0].executor_id, "sell-exec")

    async def test_update_processed_data_classifies_uptrend(self):
        import pandas as pd
        ...
        self.mock_controller_config.enable_trend_filter = True
        type(self.mock_market_data_provider).get_price_by_type = MagicMock(return_value=Decimal("148"))
        self.mock_market_data_provider.get_candles_df = MagicMock(return_value=candles)

        await self.controller.update_processed_data()
        self.assertEqual(self.controller.processed_data["trend"], "UP")
        self.assertIsNotNone(self.controller.processed_data["ema_fast"])
        self.assertGreater(self.controller.processed_data["ema_diff_pct"], Decimal("0.002"))

    async def test_update_processed_data_atr_high_widens_spreads(self):
        import pandas as pd

        n = 80
        closes = [100.0] * n
        highs = [100.1] * (n - 5) + [108.0] * 5
        lows = [99.9] * (n - 5) + [92.0] * 5
        candles = pd.DataFrame({
            "open": closes,
            "high": highs,
            "low": lows,
        ...
        await self.controller.update_processed_data()
        self.assertEqual(self.controller.processed_data["trend"], "SIDEWAYS")
        self.assertEqual(self.controller.processed_data["spread_multiplier"], Decimal("1.5"))
